...
На эту интеграцию суммарно потратили 250 тысяч рублей. В результате этой кампании 1 сентября в приложении зарегистрировался 171 человек.
...
Как нам оценить, какой из каналов привлечения сработал лучше? 
На первый взгляд, вторая кампания показала себя лучше, поскольку нам удалось привлечь больше людей за те же деньги.
Но не будем торопиться с выводами — давайте сначала проведём более подробный анализ и рассчитаем CAC для двух рекламных кампаний.
Задание:
...
with september_new_users as (SELECT user_id,
                             ...
                             FROM   (SELECT date(min(time)) as date,
                                            user_id
                                     FROM   user_actions
                                     GROUP BY user_id) t
                             WHERE  date_part('month', date) = 9
                                and date_part('year', date) = 2022), september_orders as(SELECT date(time),
                                                                user_id,
                                                                order_id
                                                         FROM   user_actions
...
FROM   (SELECT user_id,
               ads_campaign,
               count (order_id) as september_new_users_orders
        FROM   september_new_users
            LEFT JOIN september_orders using (user_id)
